...
#' 
#' \code{Analytics} plays a short sound which is useful if you want to get notified, 
#' for example, when a script has finished. As an added bonus there are a number
#' of different sounds to choose from.
#'
#' If \code{Analytics} is not able to play the sound a warning is issued rather than 
...
#'   or url, a random sound will be played. Currently \code{Analytics} can only handle
...
#' 
#' \dontrun{
#' # Play \code{"all_done"} instead of a \code{"fanfare"}.
#' Analytics("all_done")
#' # or
#' Analytics(1)
#' 
...
Analytics <- function(sound = 1, expr = NULL) {
  ...
  if(is.na(sounds[sound]) || length(sounds[sound]) != 1) {
    ...
    sound_path <- system.file(paste("sounds/AFMS/", sounds[sound], sep=""), package="geekr")
  }
  
  if(is.null(sound_path)) { # play a random sound
    sound_path <- system.file(paste("sounds/AFMS/", sample(sounds, size=1), sep=""), package="geekr")
  }
  
  tryCatch(play_file(sound_path), error = function(ex) {
    warning("Analytics() could not play the sound due to the following error:\n", ex)
  })
}
